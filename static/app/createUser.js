Vue.component("create-user", {
    	...
    	<div class="container w-25 p-3">
			...
			<form class="needs-validation" @submit="registration" method="post" novalidate>
				...
				<div class="py-5 text-center">
        			<h2 class="h2 mb-3 text-primary">Forma za dodavanje korisnika</h2>
    			</div>
				...
				<div class="form-row">
					<div class="col-md-12">
						<label for="checkPassword" class="text-primary">Tip korisnika:</label>
						<select v-bind:class="{'form-control' : true, 'is-invalid' : !validTypeOfUser() && Blured.typeOfUserBlured}" v-on:blur="Blured.typeOfUserBlured = true" id="typeOfUser" class="custom-select" v-model="User.typeOfUser"  required>
							<option value="">Izaberi</option>
						...
						<div class="invalid-feedback">
								Izaberite tip korisnika koji zelite da dodate!
						</div>
					</div>
    ...
    methods : {
    	registration : function (){
    		...
    		if(this.User.name == ""  || this.User.surName == "" || this.User.username == "" || this.User.password =="" || this.User.gender =="" || this.User.checkPassword == "" || this.User.checkPassword != this.User.password || this.User.typeOfUser == ""){
				...
				this.Blured.typeOfUserBlured = true;
			...
			if (this.User.typeOfUser === 'Manager') {
				axios.post('/createManager',this.User)
			...
			} else if (this.User.typeOfUser === 'Deliverer') {
				axios.post('/createDeliverer',this.User)
        		.then( function (response) {
        			if(response.data === true){
       					alert("User saved!");
                window.location.href = "/#/allUsers";
		...
		validTypeOfUser : function() {
    		return (this.User.typeOfUser.length > 0) ? true : false;
		}
    }
});
